...
sys_exofork(void)
{
	...
	memmove(&e->env_tf, &curenv->env_tf, sizeof(e->env_tf));
...
sys_env_set_trapframe(envid_t envid, struct Trapframe *tf)
{
	...
	user_mem_assert(e, (void *)tf, sizeof(*tf), PTE_U);
	user_mem_assert(e, (void *)tf->tf_esp, 1, PTE_U|PTE_W);
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	if(((uintptr_t)va >= UTOP) || (((uintptr_t)va & (~0 << PGSHIFT)) != (uintptr_t)va))
		return -E_INVAL;
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	if(((uintptr_t)srcva >= UTOP) || (((uintptr_t)srcva & (~0 << PGSHIFT)) != (uintptr_t)srcva))
		return -E_INVAL;

	if(((uintptr_t)dstva >= UTOP) || (((uintptr_t)dstva & (~0 << PGSHIFT)) != (uintptr_t)dstva))
		return -E_INVAL;
	...
	pte_t *pte;
	struct Page *p = page_lookup(se->env_pgdir, srcva, &pte);
	if(!p)
		return -E_INVAL;
	...
	return page_insert(de->env_pgdir, p, dstva, perm);
}
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if((uintptr_t) e->env_ipc_dstva < UTOP && (uintptr_t) srcva < UTOP){
		if(((uintptr_t) srcva & (~0 << PGSHIFT)) != (uintptr_t) srcva)
			return -E_INVAL;
		struct Page *p;
		pte_t *pte;
		...
		if((r = page_insert(e->env_pgdir, p, 
...
sys_ipc_recv(void *dstva)
{
	...
	if((uintptr_t) dstva < UTOP 
		&& (((uintptr_t) dstva & (~0 << PGSHIFT)) != (uintptr_t) dstva))
		return -E_INVAL;
